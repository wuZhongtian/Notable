...
- 在 `if` 语句和 `for` 循环的代码块中使用 JSX，将 JSX 赋值给变量，把 JSX 当作参数传入，以及从函数中返回 JSX；但不能在虚拟 DOM 中使用这些语句。
...
1. 在 redux 文件夹下 创建 count_action.js
